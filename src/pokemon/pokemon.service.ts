...
export class PokemonService {
    ...
    async findOne(term: string) 
    {
        ...
        if (!pokemon) 
            throw new NotFoundException(`Pokemon con name o no ${term} no se encuentra`);
    ...
    async remove(id: string) {
        try {
            ...
            if(deletedCount === 0)
                throw new BadRequestException(`Pokemón with id "${ id }" not found`);
    ...
    {
        if(error.code === 11000)
            throw new BadRequestException(`El Pokemón ${JSON.stringify(error.keyValue)} ya se encuentra registrado en la base de datos`);
